...
# Load the completely connected graph data
with open('graphify-out/graph_completely_connected.json', 'r') as f:
    data = json.load(f)

# Create a final comprehensive HTML file
html_template = '''<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>Complete Connected Knowledge Graph - Jarvis</title>
...
const nodes = new vis.DataSet(graphData.nodes.map(n => ({
  ...
  size: 6 + Math.min((graphData.links.filter(l => l._src === n.id || l._tgt === n.id).length * 0.2), 15),
...
const edges = new vis.DataSet(graphData.links.map(l => ({
  ...
  color: { color: getEdgeColor(l.relation), opacity: 0.6 }
})));
...
with open('graphify-out/graph_final.html', 'w') as f:
    f.write(html_template)

print("Created final comprehensive HTML visualization: graphify-out/graph_final.html")