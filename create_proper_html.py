...
# Create a simple HTML file that loads the data dynamically
html_template = '''<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>Connected Graph - Jarvis Knowledge Graph</title>
...
const nodes = new vis.DataSet(graphData.nodes.map(n => ({
  id: n.id,
  label: n.label,
  color: { background: '#4E79A7', border: '#4E79A7', highlight: { background: '#ffffff', border: '#4E79A7' } },
  size: 10 + Math.min((graphData.links.filter(l => l._src === n.id || l._tgt === n.id).length * 0.5), 10),
  font: { size: 0, color: '#ffffff' },
  title: n.label,
  community: n.community,
  community_name: `Community ${n.community}`,
...
searchInput.addEventListener('input', () => {
  ...
  matches.forEach(n => {
    ...
    el.style.borderLeft = '3px solid #4E79A7';
...
nodes.get().forEach(n => {
  if (!communities[n.community]) {
    communities[n.community] = { count: 0, color: '#4E79A7' };
  }
  communities[n.community].count++;
});

const legendEl = document.getElementById('legend');
Object.entries(communities).forEach(([comm_id, info]) => {
  const item = document.createElement('div');
  item.className = 'legend-item';
  item.innerHTML = `<div class="legend-dot" style="background:#4E79A7"></div>
    <span class="legend-label">Community ${comm_id}</span>
...
# Write the HTML file
with open('graphify-out/graph_connected_proper.html', 'w') as f:
    f.write(html_template)

print("Created properly connected HTML visualization: graphify-out/graph_connected_proper.html")